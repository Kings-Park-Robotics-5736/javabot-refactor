package frc.robot.subsystems.phoenixDrive;
...
import frc.robot.subsystems.phoenixDrive.TunerConstants.TunerSwerveDrivetrain;
...
public class DriveSubsystem extends TunerSwerveDrivetrain implements Subsystem {
     ...
     *        its default 0's.
     */
    private boolean m_joystickLockoutTranslate = false;
    private boolean m_joystickLockoutRotate = false;
    private double m_rotateLockoutValue = 0;
    private boolean m_joystickLockoutRotateFieldOriented = false;
    private double m_transXLockoutValue = 0;
    private double m_transYLockoutValue = 0;
    private boolean pathPlannerInitSuccess = false;
    ...
    public void forceStop(){
        for (int i = 0; i < 4; i++) {
            getModule(i).getDriveMotor().stopMotor();
            getModule(i).getSteerMotor().stopMotor();
        }
    }
   ...
   */
    ...
    public void drive(double xSpeed, double ySpeed, double rot, boolean fieldRelative, boolean joystick) {
        ...
        if ((joystick && !m_joystickLockoutTranslate) || !joystick) {
            ...
            super.setControl(
                fieldRelative
                    ? fieldCentricRequest.withVelocityX(xSpeed).withVelocityY(ySpeed).withRotationalRate(rot)
                    : robotCentricRequest.withVelocityX(xSpeed).withVelocityY(ySpeed).withRotationalRate(rot)
            );
        }
    ...
    public void periodic() {
        //for now, done in RobotContainer
        /*
